...
where
    F: Future<Output = io::Result<Option<(I, S)>>>,
    I: AsyncRead + AsyncWrite + Unpin,
{
    type Output = io::Result<Option<(TlsStream<I>, S)>>;
            ...
            match this.inner.as_mut().project() {
                AcceptFutureProj::Inner {
                    ...
                    match future.poll(cx) {
                        Poll::Ready(Ok(Some((stream, service)))) => {
                ...
                AcceptFutureProj::Accept { future, service } => match future.poll(cx) {
                    Poll::Ready(Ok(Ok(stream))) => {
                        ...
                        return Poll::Ready(Ok(Some((stream, service))));
                    }
